...
def start_capture(name1):
    name = name1
    path = "aligned_image_data/"+str(name)
    ...
    while (True):
        ...
        for i, rect in enumerate(rects):
            ...
            b=cv2.rectangle(frame, (bX, bY), (bX + bW, bY + bH),
                          (0, 255, 0), 1)
            ...
            if cv2.waitKey(1) & 0xFF == ord('c'):
                ...
                while i < 1:
                    ...
                    cv2.putText(frame, str("Capturing Image"), (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255),
                                4)
                    dup_frame = cv2.resize(dup_frame,(100,100))
                    cv2.imwrite(path+"/" + capture + ".jpg",
                                dup_frame    )
                    print("Capturing in process", end="", flush=True)
                    i=i+1
                    print("\nCapture  Complete")
                    cv2.putText(frame, str("Capture Complete, Starting Processing"), (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX,
                                0.8, (255, 255, 255),
                                4)
                ...
                print(path)
                import encode_build
                encode_build.run_encode(dataset_path)
